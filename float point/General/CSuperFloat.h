		   ...
		   const CSuperFloat<_FTYPE, _FCOUNT,_PTYPE>& y )
{
	...
	CGUINT<_FTYPE,_FCOUNT> *pxy, One( 1 ), temp;
	CSuperFloat<_FTYPE, _FCOUNT,_PTYPE>::_BUF a_buf, b_buf, c_buf;
	CGUINT<_FTYPE,_FCOUNT*2> xx, yy, Zero;
	...
	*(CSuperFloat<_FTYPE, _FCOUNT,_PTYPE>::_BUF*)&yy = b_buf;
	xx = xx << int( FRACTION_BITS );

	xx = xx/yy;

	pxy = (CGUINT<_FTYPE,_FCOUNT>*)&xx;

	// [4]返回值
	// 由于a_buf和b_buf均是规格化的,因此商的整数部分不可能大于1
	// 故无需在舍弃高位前做调整

	// 最高2位整数部分,此处舍弃了高FRACTION_BITS-2位部分
	c_buf.d = pxy[1];
	c_buf.d = c_buf.d << int( FRACTION_BITS - 2 );

	// 小数部分
	c_buf.d = c_buf.d | ( pxy[0] >> 2 );
