// 本模板是第6章<通用浮点运算>实现的一部分,支持8字节长度以内的任意格式的浮点数
// 格式定义来自对IEEE单精度格式和双精度格式的引申
...
{
	// 操作缓冲区,与每个参与运算的结构对应
	CFloatPoint<_TYPE,_EXP_BITS>::_BUF a_buf, b_buf;
	CGUINT<unsigned short,sizeof(_TYPE)> xx, yy;
	...
	*(CFloatPoint<_TYPE,_EXP_BITS>::_BUF*)&yy = b_buf;
	...
	xx = xx << int( sizeof(_TYPE)*8 - 2 );

	xx = xx/yy;
	*(_TYPE*)&a_buf = *(_TYPE*)&xx;
	sign = int( x.m_data.m_nSign^y.m_data.m_nSign );
