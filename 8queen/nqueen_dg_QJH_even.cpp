...
//
#include "stdafx.h"
#include "nqueen_dg_QJ_Inc_even.h"
...
//		 if (M != 4)   continue;
		 ...
		 t2 = clock();
         etime = (int)(t2-t1);
		 fprintf(logid,"----- M=%2d   %12I64d   %12I64d    %7d        Mj : %9I64d  Mn : %9I64d  /   ABcnt : %9I64d   CDcnt : %9I64d\n",
...
// 		    sprintf(str,"----qsym =%3d     %I64d  %x %x ----",qsym,Cnt,X,Y);
//    	    printNQ(str,Pos,N);
//		}
...
//		if ((Stage == 1) || (Stage == 3)) {
//			if ((X == 0x27) && (Y == 0x27)) {
// 		    fprintf(logid,"----qsym =%3d     %I64d  %x %x %x %x ----\n",qsym,Cnt,X,Y,U,V);
// 		    sprintf(str,"----qsym =%3d     %I64d  %x %x %x %x   %I64x  %I64x----",qsym,Cnt,X,Y,U,V,Qpos(0),Qpos(-qsym));
//    	    printNQ(str,Pos,N);
//			}
//		}
		if (qsym < 0)                      return;
