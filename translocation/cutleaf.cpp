...
int getNonLeaf(const Permutation& permut, const ChromRange& chromrg1, const Component& comps, vector<TransParam>& trsprms)
{
    int transNumber=0;
    ChromRange chromrg2;
  ...
  //std::cout<<"?cut=?"<<chromrg2<<"\n";
            ...
            list<ChromTree::ChromTreeNode*>::const_iterator cctitor2=comps.m_chromTrees[chj].trees.begin();
            for ( ;cctitor2!=comps.m_chromTrees[chj].trees.end(); ++cctitor2) {
                //vector<Interval >::const_iterator cpitor2=(*cctitor2)->leafs.begin();
...
int getAllCutTransParam(const Permutation& permut, const Component& comps, vector<TransParam>& trsprms)
{

	int transNumber=0;//, ,-
	ChromRange chromrg1;
	list<ChromTree::ChromTreeNode*>::const_iterator cctitor1,cctitor1e;
	vector<Interval >::const_iterator cpitor1;
  ...
  //std::cout<<"?cut=? ?"<<chromrg1<<"\n";
	...
	if ((comps.m_totalLeafNumber%2==0)&&(comps.m_totalTreeNumber==1)){
        ...
        cctitor1=comps.m_chromTrees[chi].trees.begin();
        chromrg1.chromIndex=chi;
		chromrg1.startIndex=(*cctitor1)->intvl.begin;
...
//std::cout<<"?cut=?adjacency?"<<chromrg1<<"\n";
        ...
        --cpitor1;
        chromrg1.startIndex=(*cpitor1).end;
		chromrg1.endIndex=(*cctitor1)->intvl.end;
#ifdef _DEBUG_OUT
